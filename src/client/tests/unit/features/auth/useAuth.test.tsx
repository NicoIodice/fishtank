...
describe('useAuth', () => {
  it('returns user data on successful /api/auth/me', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ success: true, data: { username: 'admin', role: 'Admin', forcePasswordChange: false } }),
    ...
    expect(result.current.user).toEqual({ username: 'admin', role: 'Admin', forcePasswordChange: false })
  ...
  it('returns null user on 401 (not authenticated)', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ success: false, error: { code: 'AUTH_UNAUTHORIZED', message: 'Not authenticated' } }),
  ...
  it('does NOT redirect to /login on 401 (uses redirectOn401:false)', async () => {
    ...
    Object.defineProperty(window, 'location', {
      value: { ...original, set href(_: string) { redirectFired = true } },
      writable: true,
    })
  ...
  it('isLoading is true while fetching', () => {
    ...
    resolveFetch(new Response(JSON.stringify({ success: true, data: null }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
  })
