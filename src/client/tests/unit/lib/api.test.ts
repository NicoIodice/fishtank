...
describe('apiFetch', () => {
  ...
  it('sends credentials: include by default', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('/api/test', expect.objectContaining({ credentials: 'include' }))
  })
  ...
  it('throws ApiError with AUTH_UNAUTHORIZED on 401', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockResponse({ success: false, error: { code: 'AUTH_UNAUTHORIZED', message: 'Not authenticated' } }, 401),
    )

    await expect(apiFetch('/api/protected')).rejects.toSatisfy(
      (e: unknown) => e instanceof ApiError && (e as ApiError).code === 'AUTH_UNAUTHORIZED',
    )
  })
  ...
  it('surfaces error code and message from 401 body', async () => {
    ...
    const err = await apiFetch('/api/auth/login', { method: 'POST', redirectOn401: false }).catch(e => e) as ApiError
  ...
  it('throws ApiError with HTTP_404 on 404 response with JSON body', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockResponse({ success: false, error: { code: 'NOT_FOUND', message: 'Resource not found' } }, 404),
    )
  ...
  it('throws ApiError with HTTP_500 when server returns 500 with JSON error', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockResponse({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Unexpected server error' } }, 500),
    )
  ...
  it('throws ApiError when success=false on a 200 response', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockResponse({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid input' } }, 200),
    )
