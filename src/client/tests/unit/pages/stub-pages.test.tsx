...
vi.mock("@/features/auth/hooks/useAuth", () => ({ useAuth: () => mockUseAuth() }));
vi.mock("@/features/admin/hooks/useTogglesHub", () => ({ useTogglesHub: vi.fn() }));
vi.mock("@/features/admin/hooks/useToggles", () => ({
  useToggles: () => ({ toggles: [], isLoading: false, error: null, setToggle: vi.fn(), isSettingToggle: false }),
}));

describe("AdminPage", () => {
  beforeEach(() => {
    mockUseAuth.mockReturnValue({ user: { id: "1", username: "admin", role: "Admin" }, isAuthenticated: true, isLoading: false });
  });
  ...
  it("renders null for Standard User (role guard)", () => {
    mockUseAuth.mockReturnValue({ user: { id: "2", username: "std", role: "Standard User" }, isAuthenticated: true, isLoading: false });
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(
