...
- **Collapsed on touch devices:** tapping a nav item navigates and simultaneously expands the sidebar to reveal labels — hover tooltips are not shown on touch
...
- **Dismiss (✕):** removes the item from the panel view only — the event **remains in System Events** and is considered read. Dismissed items do not reappear unless System Events are cleared and re-generated.
...
### Toolbar
Left to right:
1. **Search field** — magnifying glass icon on left, clear (×) button on right, filters URL path + method. **Matching semantics:** case-insensitive, contains-match; OR logic across fields — a row matches if the URL path **or** the HTTP method label contains the query string. Searches the full URL path value, not the truncated display. Example: typing “post” returns rows where method = POST and rows where the path contains “post”.
...
- Click → popover anchored below-left of the pill, max-width 240px, `{rounded.card}`, elevation level 2. Content: list of service display names with their individual proxied counts (e.g. "Finance API — 14"). Services with 0 proxied requests are omitted. Dismiss: click outside, Esc, or second click on the pill.
...
**Bottom panel:** split-view below table, tab bar (Request / Response / Headers), close collapses panel. For proxied requests, the “Save as Mock” button is pinned at the top-right of the panel header, beside the Close button.
...
- Service Name (text, required; max 64 characters; any printable characters excluding emojis; must produce a unique slug — if the generated slug collides with an existing service, an inline error is shown: "A service with this name already exists."; validated on blur and on submit)
...
New services are created in **Live** state — the mock server starts listening on the assigned port immediately after the modal closes.

The three path fields update in real time as the user types the service name. They are never editable directly.

**Edit Service modal:** opens when clicking “Edit” on a service card or the `bi-pencil` action in the services table view. Fields are identical to Add Service (Name, Port, Description, External URL) with current values pre-filled. The three read-only path fields update live as the user edits the name. **Renaming a service changes its slug**, which changes the Mocks Root path. If the service has existing files on disk, an inline warning is shown in the modal before saving: “Renaming this service will change its Mocks Root path from `/mocks/{old-slug}/` to `/mocks/{new-slug}/`. Existing mapping and response files will **not** be moved automatically — you must rename the directory on disk and run Resync to reload.” The save proceeds regardless; the user is responsible for migrating files on disk. Port validation is identical to Add Service (30100–30199, must not collide with another service’s port — if a collision is detected, inline error: “Port {N} is already in use by {service-name}.”).
...
| Enable | Enable/disable toggle — same optimistic behavior and rollback rules as card toggle |
| Actions | `bi-pencil` (tooltip: “Edit”) + `bi-file-earmark-code` (tooltip: “Mappings”) — icon-only |

- Column sorting: Name, Port, Mock Count, Status columns are sortable. Click cycles: unsorted → ascending → descending. One column sorted at a time.
- Stopped service rows: `{components.service-card-stopped-opacity}` opacity (matches card behavior).
- Empty state: same as card grid — `bi-server` (48px, muted) + “No services yet.” + primary **Add Service** button.
...
│  │  └ 📄 acct-post-201.json●│  └───────────────────────────────────────────┘   │
│  └ 📁 responses/            │  [Duplicate] [Delete]  [Discard*] [Save*]             │
│ 📁 Social Security          │                                                   │
...
- Context menu (right-click / long-press): **Rename**, **Duplicate** (copies the file to the same sub-folder), **Delete**. "Duplicate" opens the same naming modal as the editor footer Duplicate button.

**File actions:**
- **View:** single click on file node loads read-only preview; while the file is loading, the editor pane shows a single-line skeleton placeholder (file loads in < 100ms for typical mapping files — no spinner is needed)
...
- **Copy from existing:** “Duplicate” button in editor footer → modal: source filename pre-filled, user enters new name → copies file to same sub-folder → opens copy in editor in edit mode
- **Rename:** only available via context menu (right-click / long-press on a file node in the tree). A modal opens with the current filename pre-filled in an editable input. Validation: must be unique in its sub-folder (same inline error as new-file collision), must end in `.json`, must not be empty. On confirm: file is renamed on disk, folder tree node updates, and the editor breadcrumb updates if that file is currently open. **If the file being renamed is a response file** referenced by a `BodyAsFile` field in a mapping, a warning banner is shown after saving: “This file is referenced as `BodyAsFile` in one or more mappings. Update those mappings manually to use the new filename.” No automatic reference update occurs.
- **Network Activity → Mappings:** when "Save" is triggered in the Mock Suggestion modal, the mapping file is written to `mappings/` and the response file to `responses/`; the folder tree re-fetch is **scheduled** — it runs the next time the user navigates to the Mappings screen, not immediately from the Network Activity screen.
...
**Path slugification rule:** replace `/` with `_`, strip leading `_`, replace path parameters (`{id}`, `:id`) with the literal string `param`, collapse consecutive `_` to single `_`, lowercase all. Example: `/api/v1/users/{id}/orders` → `api_v1_users_param_orders`.
...
**Navigation-away guard (unsaved changes):** when the current file has unsaved changes, a confirmation dialog — “Discard unsaved changes in `{filename}`? This cannot be undone.” — is shown before any of the following:
...
**Per-item dismiss (✕):** dismissing an item in System Events removes it from the current view only — the server-side event log is unchanged and the event reappears on page reload. Use "Clear all" to permanently remove events from the server log.
...
- Sidebar default state: select (Expanded / Collapsed). Defines the initial sidebar state on first visit or after localStorage is cleared. When the user has manually toggled the sidebar (state persisted in localStorage), the localStorage value takes precedence over this setting on all subsequent loads. **Note:** this setting only takes effect on first load or after localStorage is cleared — once a user has manually toggled the sidebar, localStorage persists that preference indefinitely and this setting is overridden.
...
**Paths section:** Mocks Root path — single text input showing the current configured path. Before applying a change:
...
- Named caches list: each service cache shown as a row with name, entry count, estimated size, and individual `Clear` button → confirmation dialog ("Clear cache for {service-name}? Cached mappings will be reloaded from disk on next request. This cannot be undone." Actions: **Cancel** and **Clear cache**)
...
**Feature Flags section:** **Record Mode enabled** toggle (enables/disables the Record Mode feature globally — when off, the Record button is hidden from the Network Activity page header), **Pipeline Reset API** toggle (enables the `POST /admin/reset` HTTP endpoint on the Fishtank server, allowing clients to trigger a full in-memory state reset equivalent to a service restart without a container restart; disabled by default — enable only in controlled environments). **If Record Mode is active when the feature flag is toggled off**, recording stops immediately: the Recording badge is hidden, the activity log retains all rows captured so far, and a warning toast appears: "Record Mode was disabled. Recording stopped."

## Login screen
Centered card on plain background (sidebar hidden). Fields: username (`autocomplete="username"`), password (`autocomplete="current-password"`). Primary CTA: "Sign in". No "forgot password" (self-hosted, admin resets via env var). Shows Fishtank logo + version. **Logo gate:** the logo on the login card uses the same placeholder (`bi-droplet-half`) subject to the hard v1 release gate in DESIGN.md Brand & Style — the final custom SVG must be in place before this screen ships. Both fields: `aria-required="true"`. Error on failed login: inline message below the form — "Invalid username or password." Never indicate which field is wrong (prevents username enumeration). **Rate limiting and account lockout are out of v1 scope** — this is a developer tool deployed on trusted internal networks; document this decision in the security assumptions.
...
- Confirm password: must match password (`autocomplete="new-password"`). Inline error on blur: “Passwords do not match.”
- All validation re-runs on submit before the request is sent.
- On server error during account creation, an inline error is shown below the form: "Account creation failed — {reason}. Try again." The form remains active.
---
...
| Record mode active | Network Activity **page header** shows persistent "● Recording" badge in warning/amber color; **Record button** shows active state (label: "⏹ Stop") |
| Notification bell unread | Red badge with count; animates in when new events arrive (scale pulse) |
| Backend unreachable | Persistent amber banner in top bar: "Backend unreachable — retrying…"; write operations disabled (buttons disabled, tooltip: "Offline — changes cannot be saved"); tables and cards show last-known data with a muted "Last updated N ago" suffix in the page header; banner auto-dismisses when connection is restored and a **Success** toast (auto-dismisses after 4s) confirms "Reconnected." If any write was blocked while offline, no automatic retry occurs — users with unsaved mapping changes will still see the `●` unsaved indicator; other blocked writes (service toggles, settings changes) are lost silently and must be re-applied. |
...
**Toasts:** bottom-right, stack vertically (newest on top), max 3 visible, auto-dismiss 4s for success/info (info toasts may be pinned for in-progress operations — see DESIGN.md toast table), 6s for warnings (errors persist until dismissed manually).
...
- Notification badge: `aria-label="{N} unread warnings and errors"` when count ≤ 99; `aria-label="More than 99 unread warnings and errors"` when count > 99. Updated dynamically on badge change.
