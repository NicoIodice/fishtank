...
colors:
  ...
  icon-mock-color:        "#3b82f6"   # blue-500 — bi-database; intentionally matches {colors.info} — update both if one changes
  icon-proxy-color:       "#10b981"   # emerald-500 — bi-arrow-repeat; intentionally matches {colors.method-post} — update both if one changes
  ...
  # ⚠ Known visual overlap: a PUT proxied live row shows both an amber PUT chip (method-put: amber-600 / method-put-bg: amber-200) and this amber-500 left-border. Accept as intentional tradeoff or adjust one shade before v1 ships.
...
spacing:
  ...
  nav-item-py:            "8px"
  toolbar-py:             "12px"
  section-gap:            "16px"    # 4 × 4px — aligned to 4px base unit
...
components:
  ...
  topbar-badge-size:       "16px"   # 4 × 4px — aligned to 4px base unit
---
...
**Logo mark:** Bootstrap Icon `bi-droplet-half` (placeholder). A custom SVG logo must be designed before v1 ships — this is a **hard v1 release gate**: "Custom SVG logo reviewed and merged" must appear as a completed task in the v1 Definition of Done before the build is tagged for release. The placeholder `bi-droplet-half` must not ship as the final identity. **Gate owner:** product lead (must be assigned before v1 kickoff). **Acceptance criteria:** single-color SVG approved by product lead, merged to main branch, favicon generated and verified at 16×16px. Design constraints for the final logo: rendered at 20×20px beside the wordmark; must be a single-color SVG that accepts a `fill` CSS property so it adapts to the active theme's `{colors.brand-fg}`; must remain legible at 16×16px (favicon use); must have a monochrome fallback for print/high-contrast contexts.
...
**Sidebar collapse:** `{spacing.sidebar-transition}`. **Collapse chevron:** `transform: rotate(180deg)` with matching 200ms ease transition.
...
- Icon-only buttons (top bar, toolbar, sidebar collapse): `{rounded.xl}` — larger touch target than text buttons
...
**Service slug convention:** lowercase display name; consecutive spaces collapse to a single hyphen; spaces → hyphens; leading/trailing spaces trimmed before slugification; non-alphanumeric non-hyphen characters stripped; consecutive hyphens collapsed to one. Example: "Finance API" → `finance-api`; "Finance & Loans API" → `finance-loans-api`.
...
- **Warning (≤ 10 ports remaining):** the port input field shows an amber inline notice: "X ports remaining in range (30100–30199)." Style: `{colors.warning}` text + `{colors.warning-subtle}` background, `{rounded.md}`, `{typography.scale.sm}`, `bi-exclamation-triangle` leading icon, displayed directly below the port input field.
- **Range full (0 ports remaining):** the Add Service button is disabled; an amber full-width banner in the Services view reads: "Port range 30100–30199 is fully allocated. Remove a service to free a port." Banner style: `{colors.warning}` text + `{colors.warning-subtle}` background, `{spacing.page-px}` horizontal padding, `bi-exclamation-triangle` leading icon. The port field is hidden from the Add Service form.
...
> **Auth & Users — not in v1 scope.** User management, roles, and authentication configuration are intentionally excluded from v1. The nav item is rendered at 50% opacity with a `bi-lock` icon appended; clicking it shows a centered placeholder reading "Auth & Users — coming soon" with muted body text "User management will be available in a future release." No form fields, no action buttons. **This nav item must be removed (not hidden) before v2 ships unless a dedicated Auth & Users spec has been written and approved.**

**Cache section:** Visual layout only — see EXPERIENCE.md Settings screen for full behavioral spec.
- “Clear all caches” row: label + description text (left) + red `Clear All` button (right)
...
- Top bar icon buttons (shows action label: “About”, “Notifications”)
- Network Activity type icon (`bi-database` → “Served from mock”; `bi-arrow-repeat` → “Proxied to external”)
...
| Emerald Terminal | ✅ Pass | `--content-muted` (#64748b) on `#f9fafb` ≈ 4.6:1 — passes; `--sidebar-fg` (#6ee7b7) on `#064e3b` ≈ 5.96:1 — passes WCAG AA |
...
| Enable/disable toggle (table view) | `services-table-toggle-{slug}` |
| Edit button on card | `services-btn-edit-{slug}` |
| Mappings link on card | `services-btn-mappings-{slug}` |
...
| Folder tree node (per file) | `mappings-tree-node-{filename}` |
...
| Settings theme swatch (per theme) | `settings-swatch-{theme-slug}` |
...
| System Events clear all button | `events-btn-clear-all` |
